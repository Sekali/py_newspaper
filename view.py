...
date = {}
date['year'] = strftime("%Y-%m", localtime())
date['day'] = strftime("%d", localtime())
...
def index_get(name): #用于获取当日的报纸
    img_list = []
    baozhi_h = {}
    page = urlopen("http://www.zgfznews.com/paper/")
    html = page.read()
...
def text(name, a, b, c): #文章显示页面
    ...
    for i in context.find_all('img'):
        i['class'] = 'img-responsive center-block'
